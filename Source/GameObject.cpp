...
void CSheildObject::Render(HDC hDCFrameBuffer, CCamera* pCamera) 
{
	if (m_bActive) 
	{
		m_pMesh->Render(hDCFrameBuffer);
...
void CSheildObject::Animate(float fElapsedTime)
{
	...
	{
		m_bActive = false;
		m_fElapsedFromLastActive = 0.f;
	}
}

void CSheildObject::AfterCollision(const CGameObject* pObjectCollided)
{
	m_bActive = false;
	m_fElapsedFromLastActive = 0.f;
}

void CSheildObject::ActiveSheild()
{
	if (m_bActive || m_fElapsedFromLastActive < m_fActiveLimitTime) 
	{
		return;
	}
